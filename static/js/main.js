...
function loadBookmarks() {
    const savedBookmarks = localStorage.getItem('bookmarkedApps');
    if (savedBookmarks) {
        bookmarkedApps = JSON.parse(savedBookmarks);
    }
}

function saveBookmarks() {
    localStorage.setItem('bookmarkedApps', JSON.stringify(bookmarkedApps));
}

function findApp(appName, sectionName) {
    if (sectionName && appData[sectionName] && appData[sectionName].apps) {
        const app = appData[sectionName].apps.find(a => a.name === appName);
        if (app) return { ...app, section: sectionName };
    }
    for (const section in appData) {
        if (appData[section].apps) {
            const app = appData[section].apps.find(a => a.name === appName);
            if (app) return { ...app, section: section };
        }
    }
    return null;
}

function toggleBookmark(appName, sectionName) {
    const bookmarkIndex = bookmarkedApps.findIndex(b => b.name === appName);
    if (bookmarkIndex > -1) {
        bookmarkedApps.splice(bookmarkIndex, 1);
    } else {
        bookmarkedApps.push({ name: appName, section: sectionName });
    }
    saveBookmarks();
    updateBookmarksSection();
}

function initializeBookmarksSection() {
    const sectionId = 'bookmarks';
...
function updateBookmarksSection() {
    ...
    const appsToRender = bookmarkedApps.map(bookmark => findApp(bookmark.name, bookmark.section)).filter(app => app !== null);
    
    generateAppCards(sectionId, grid, appsToRender);
...
function setupBookmarkEventListeners(container) {
    const bookmarkCheckboxes = container.querySelectorAll('.ui-bookmark input[type="checkbox"]');
    bookmarkCheckboxes.forEach(checkbox => {
        ...
        checkbox.addEventListener('change', function() {
            ...
            toggleBookmark(appName, sectionName);

            const allCheckboxesForThisApp = document.querySelectorAll(`.ui-bookmark input[data-app-name="${appName}"]`);
            allCheckboxesForThisApp.forEach(cb => {
                if (cb !== this) {
...
function setupEmptyStateHandling(sectionId) {
    ...
    let emptyState = grid.parentNode.querySelector('.empty-state');
    if (!emptyState) {
    ...
    if (!document.emptyStateListeners) {
        document.emptyStateListeners = new Set();
    }
    if (!document.emptyStateListeners.has(sectionId)) {
        document.addEventListener('filterComplete', function(event) {
            if (event.detail.sectionId === sectionId) {
                checkEmptyState();
            }
        });
        document.emptyStateListeners.add(sectionId);
    }
     checkEmptyState();
}
...
function generateAppCards(sectionId, container, apps) {
    ...
    apps.forEach(app => {
        ...
        const isBookmarked = bookmarkedApps.some(b => b.name === app.name);
        const appSection = app.section || sectionId;

        const bookmarkHTML = `
            <label class="ui-bookmark">
                <input type="checkbox" data-app-name="${app.name}" data-section-name="${appSection}" ${isBookmarked ? 'checked' : ''}>
                <div class="bookmark">
        ...
        <div class="app-info">
            ...
            ${app.originalUrl ?
            `<a href="${app.originalUrl}" class="app-description" target="_blank">Original</a>` : ''}
            <div class="app-meta">
    ...
    setupBookmarkEventListeners(container); // [НОВЕ]
}
    ...
    if (selectedCategories.length === 0) {
        ...
        appCards.forEach(card => {
            ...
            if (app) {
                ...
                const 
 isVisible = selectedCategories.every(cat => appCategories.includes(cat));
